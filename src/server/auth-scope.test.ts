...
import { currentUserId, ForbiddenOrNotFoundError, requireTrip, UnauthenticatedError } from './auth-scope';
...
describe('requireTrip', () => {
  it('returns the trip when owned by the current user', async () => {
    ...
    expect(db.trip.findFirst).toHaveBeenCalledWith({ where: { id: 'trip-1', userId: 'user-1' } });
  });
  ...
  it('throws ForbiddenOrNotFoundError when the trip is not owned or does not exist', async () => {
    ...
    await expect(requireTrip('trip-2')).rejects.toBeInstanceOf(ForbiddenOrNotFoundError);
  });

  it('throws UnauthenticatedError when there is no session, without querying the trip', async () => {
    vi.mocked(auth).mockResolvedValue(null as never);

    await expect(requireTrip('trip-1')).rejects.toBeInstanceOf(UnauthenticatedError);
