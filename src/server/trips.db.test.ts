...
beforeEach(async () => {
  const user = await db.user.create({ data: { email: `trips-db-test-${crypto.randomUUID()}@example.com` } });
  userId = user.id;
...
describe('trips against a real database', () => {
  ...
  it('cascade-deletes days/activities/expenses when a trip is deleted', async () => {
    ...
    const day = await db.day.create({ data: { tripId: trip.id, date: validInput.startDate } });
    await db.activity.create({ data: { dayId: day.id, title: 'Arrive', category: 'Transport', sortOrder: 0 } });
    await db.expense.create({
      data: { tripId: trip.id, label: 'Flights', category: 'Transport', costMinor: 1000000, costCurrency: 'JPY' },
    });

    await db.trip.delete({ where: { id: trip.id } });

    expect(await db.day.findMany({ where: { tripId: trip.id } })).toHaveLength(0);
    expect(await db.expense.findMany({ where: { tripId: trip.id } })).toHaveLength(0);
  });
  ...
  it('rejects a stale update once the row has actually changed', async () => {
    ...
    await db.trip.update({ where: { id: trip.id }, data: { name: 'Renamed elsewhere' } });
  ...
  it('applies an update when the optimistic lock is current', async () => {
    ...
    await updateTrip(trip.id, { ...validInput, name: 'Renamed', updatedAt: trip.updatedAt });
  ...
  it("rejects requireTrip-gated access to another user's trip", async () => {
    ...
    try {
      ...
      await expect(
        updateTrip(otherTrip.id, { ...validInput, updatedAt: otherTrip.updatedAt }),
