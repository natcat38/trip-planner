...
vi.mock('../lib/db', () => ({
  db: { expense: { create: vi.fn(), findFirst: vi.fn(), delete: vi.fn() } },
}));
...
describe('createExpense', () => {
  it('converts the cost to minor units and creates the expense', async () => {
    ...
    await createExpense('trip-1', { label: 'Flights', category: 'Transport', costAmount: 60, costCurrency: 'EUR' });

    expect(db.expense.create).toHaveBeenCalledWith({
      data: { tripId: 'trip-1', label: 'Flights', category: 'Transport', costMinor: 6000, costCurrency: 'EUR' },
    });
  });
  ...
  it('rejects a negative cost amount', async () => {
    ...
    await expect(
      createExpense('trip-1', { label: 'Flights', category: 'Transport', costAmount: -1, costCurrency: 'EUR' }),
...
describe('deleteExpense', () => {
  it('deletes the expense after authorization', async () => {
    vi.mocked(requireTrip).mockResolvedValue(trip as never);
    vi.mocked(db.expense.findFirst).mockResolvedValue({ id: 'expense-1', tripId: 'trip-1' } as never);
    vi.mocked(db.expense.delete).mockResolvedValue({} as never);

    await deleteExpense('trip-1', 'expense-1');

    expect(db.expense.delete).toHaveBeenCalledWith({ where: { id: 'expense-1' } });
  });
  ...
  it("throws ForbiddenOrNotFoundError when the expense isn't scoped to the trip", async () => {
    ...
    await expect(deleteExpense('trip-1', 'expense-1')).rejects.toBeInstanceOf(ForbiddenOrNotFoundError);
  });
});
