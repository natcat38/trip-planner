...
| `src/components` | 8 | The universal `rounded-lg border p-5` container — the panel wrapper that appears, byte-for-byte identical, around most of a trip page's sections (Budget, Sharing, Places, the settings panels, the shared-trip view). |
| `src/lib` | 16 | Byte-size formatting, shared by the attachment server module and its client component. |
...
25 source directories, 113 files, 0 without a declared purpose.
